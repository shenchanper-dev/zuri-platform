#!/bin/bash
echo "Arreglando sintaxis de Gestión Excel..."

cat > src/app/dashboard/gestion-excel/page.tsx << 'EOFPAGE'
"use client";
  ...
  const guardar = async (id: number, forzarConReserva: boolean) => {
    ...
    await fetch(`/api/solicitudes-servicios/${id}`, {
      ...
      body: JSON.stringify({
        ...
        conflicto_detalle: forzarConReserva ? 'RESERVADO - POR CORREGIR: Conflicto de horario' : null
      })
    });
    await cargarDatos();
    setEditando(null);
        ...
        <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    {editando?.id === s.id ? (
                      <div className="space-y-3">
                        ...
                        <div><label className="block text-xs font-medium mb-1">Conductor</label>
                          ...
                          {verificandoConflicto && <p className="text-xs text-blue-600 mt-1">Verificando...</p>}
                        </div>
                        <div className="flex gap-2 justify-end">
                          <button onClick={() => setEditando(null)} className="px-3 py-1.5 text-sm border rounded">Cancelar</button>
                          <button onClick={() => intentarGuardar(s.id)} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded" disabled={verificandoConflicto}>Guardar</button>
                        </div>
                      </div>
                    ) : (
                      <div className="flex justify-between items-start">
                        ...
                        <button onClick={() => setEditando({id: s.id, fecha: s.fecha_servicio, hora_inicio: s.hora_inicio, hora_fin: s.hora_fin, conductor_id: s.conductor_db_id || ''})} className="text-blue-600 text-sm font-medium ml-4">Editar</button>
                      </div>
                    )}
                  </div>
                ))}
          ...
          <div className="px-6 py-4 border-t bg-gray-50 flex justify-between">
            <span className="text-sm text-gray-600">
              ...
              {servicios.filter(s => s.tiene_conflicto).length} conflictos
            </span>
            <button onClick={onClose} className="px-4 py-2 text-sm bg-blue-600 text-white rounded">Cerrar</button>
          </div>
        </div>
      </div>
      ...
      {mostrarModalConflicto && conflicto && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[60]">
          <div className="bg-white rounded-lg max-w-2xl w-full mx-4">
            ...
            <div className="p-6">
              <p>El conductor <strong>{conflicto.conductorNombre}</strong> ya tiene servicios programados en este horario.</p>
            </div>
            <div className="px-6 py-4 border-t flex justify-end gap-3">
              ...
              <button onClick={() => guardar(conflicto.servicioId, true)} className="px-4 py-2 text-sm bg-orange-600 text-white rounded">Continuar RESERVADO</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Importa y genera programaciones</p>
      </div>

      {notification && (
        <div className={`fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg ${notification.type === 'success' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
      ...
      {estadisticas && (
        <div className="grid grid-cols-4 gap-6 mb-8">
          {[
            { label: 'Importaciones', value: estadisticas.total_importaciones },
            { label: 'Servicios', value: estadisticas.total_servicios },
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="px-6 py-4 border-b"><h2 className="text-lg font-semibold">Historial</h2></div>
          ...
          <tbody className="divide-y">
            {importaciones.map(imp => (
              <tr key={imp.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4"><p className="text-sm truncate max-w-xs">{imp.nombre_archivo}</p></td>
                ...
                <td className="px-6 py-4 text-sm">{imp.registros_procesados}/{imp.total_registros}</td>
                <td className="px-6 py-4">
                  <div className="flex gap-2">
                    <button onClick={() => setModalEdicion(imp)} className="text-blue-600 hover:text-blue-800 text-sm">Editar</button>
                    <button onClick={() => generarProgramacion(imp.id)} className="text-green-600 hover:text-green-800 text-sm" disabled={generando}>
                      {generando ? 'Generando...' : 'Programar'}
                    </button>
                    <button onClick={() => confirm('¿Eliminar?') && deleteImportacion(imp.id)} className="text-red-600 hover:text-red-800 text-sm">Eliminar</button>
                  </div>
                </td>
              </tr>
            ))}
...
EOFPAGEcat > fix_gestion_excel_syntax.sh << 'FIXSCRIPT'
#!/bin/bash
echo "Arreglando sintaxis de Gestión Excel..."
...
echo "Sintaxis corregida y compilado"
