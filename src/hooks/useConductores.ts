...
const API_CONFIG = {
  BASE_URL: process.env.NEXT_PUBLIC_API_URL || 'https://admin.zuri.pe/api',
  TIMEOUT: 10000,
  RETRY_ATTEMPTS: 3,
} as const;
  ...
  const filtrarConductoresLocal = useCallback((filtros: {
    ...
    return conductores.filter(conductor => {
      ...
      if (filtros.busqueda) {
        ...
        return (
          match(conductor.nombreCompleto) ||     // Required field
          match(conductor.dni) ||                // Required field  
          match(conductor.celular1) ||           // Required field
          match(conductor.placaVehiculo) ||      // ✅ CRÍTICO: Ahora null-safe
          match(conductor.marcaAuto) ||          // ✅ Null-safe
          match(conductor.modeloAuto) ||         // ✅ Null-safe
          match(conductor.distrito_nombre) ||    // ✅ Null-safe
          match(conductor.email)                 // ✅ Null-safe
        );
      }
