...
const io = new SocketIOServer(httpServer, {
  ...
  transports: ['polling', 'websocket'], // polling primero según solicitud staff
  pingInterval: 25000,
  pingTimeout: 20000,
...
io.on('connection', (socket) => {
  ...
  console.log(`[${timestamp}] 🔌 New connection attempt (socket: ${socket.id}, role: ${socket.data.role})`);

  if (socket.data.role === 'driver') {
    console.log(`[${timestamp}] 🚗 Driver connected: ${conductorId} (socket: ${socket.id})`);
  }

  // Store active connection (solo para drivers, no dashboard)
  if (socket.data.role === 'driver') {
  ...
  socket.on('disconnect', (reason) => {
    ...
    console.log(`[${ts}] 🚗 Driver disconnected: ${conductorId} (socket: ${socket.id}, reason: ${reason})`);
