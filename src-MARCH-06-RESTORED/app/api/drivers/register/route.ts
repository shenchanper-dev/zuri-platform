...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const existeQuery = `
      SELECT id, dni, email, celular1, placa, estado, estado_registro
      FROM conductores
    ...
    `;
        ...
        if (existeResult.rows.length > 0) {
            ...
            // Si el conductor previo está INACTIVO (rechazado o nunca aprobado),
            // eliminar el registro viejo sin rastro y permitir re-registro.
            // Cubre: admin rechazó conductor → conductor intenta registrarse de nuevo.
            const esInactivo = existe.estado === 'INACTIVO';
            const estadoPermiteReregistro = ['RECHAZADO', 'EN_PROCESO'].includes(existe.estado_registro);

            if (esInactivo && estadoPermiteReregistro) {
                await client.query(`UPDATE programacion_detalles SET conductor_id = NULL WHERE conductor_id = $1`, [existe.id]);
                await client.query(`UPDATE solicitudes_servicios SET conductor_id = NULL WHERE conductor_id = $1`, [existe.id]);
                await client.query(`UPDATE servicios SET "conductorId" = NULL WHERE "conductorId" = $1`, [existe.id]);
                await client.query(`UPDATE toma_muestras SET conductor_id = NULL WHERE conductor_id = $1`, [existe.id]);
                await client.query(`DELETE FROM conductores WHERE id = $1`, [existe.id]);
                console.log(`🗑️ [Register] Registro previo inactivo eliminado (ID: ${existe.id}) para permitir re-registro`);
                // Continuar con el INSERT normal
            } else {
                // Conductor activo o aprobado — bloquear duplicado
    ...
    `;
        ...
        // Enviar email con código de verificación (fire-and-forget: no bloquea la respuesta)
        import('@/lib/notifications').then(({ sendVerificationCodeEmail, sendNewDriverAdminAlert }) => {
            // 1. Email de verificación al conductor
            sendVerificationCodeEmail(data.email, data.nombres, codigoEmail).catch((err: Error) =>
                console.error('❌ [Register] Email error (non-blocking):', err.message)
            );
            // 2. Alerta al equipo admin: email + WhatsApp
            sendNewDriverAdminAlert({
                nombres: conductor.nombres,
                apellidos: conductor.apellidos,
                dni: conductor.dni,
                email: conductor.email,
                celular: conductor.celular1,
                conductorId: conductor.id,
            }).catch((err: Error) =>
                console.error('❌ [Register] Admin alert error (non-blocking):', err.message)
            );
        });
