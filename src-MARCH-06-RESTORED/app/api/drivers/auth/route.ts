...
export async function POST(request: NextRequest) {
    ...
    `, [dni]);
        ...
        // 1. Si el conductor está ACTIVO, puede hacer login siempre
        //    (el admin lo activó manualmente, sin importar estado_registro)
        // 2. Si NO está ACTIVO, verificar estado_registro para dar mensaje apropiado
        if (c.estado !== 'ACTIVO') {
            // Verificar estado de registro para mensaje informativo
            if (c.estado_registro !== 'APROBADO') {
                ...
                return jsonResponse({
                    success: false,
                    error: mensajes[c.estado_registro as string] || 'Tu solicitud está siendo revisada.',
                    estadoRegistro: c.estado_registro,
                    razonRechazo: c.razon_rechazo
                }, { status: 403 });
            }
            // Tiene estado_registro APROBADO pero estado != ACTIVO (suspendido, etc.)
            return jsonResponse(
                { success: false, error: `Conductor ${(c.estado || '').toLowerCase()}. Contacte al administrador.` },
                { status: 403 }
            );
        }
        // Conductor ACTIVO → puede autenticarse (continúa el flujo de PIN)
