        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredConductores.map((c: any) => {
              const isPendingApproval = c.estado_registro === 'PENDIENTE' || c.estado_registro === 'EN_PROCESO';
              return (
                <tr key={c.id} className={`transition-all duration-200 group ${isPendingApproval ? 'bg-red-50/50 hover:bg-red-50' : 'hover:bg-blue-50/30'}`}>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-full overflow-hidden shadow-sm bg-slate-100 flex items-center justify-center flex-shrink-0 ${isPendingApproval ? 'border-2 border-red-300' : 'border-2 border-blue-100'}`}>
                        {c.foto ? (
                          <img
                        ...
                        <div className={`${c.foto ? 'hidden' : 'flex'} w-full h-full items-center justify-center ${isPendingApproval ? 'bg-gradient-to-br from-red-400 to-rose-500' : 'bg-gradient-to-br from-blue-500 to-indigo-600'} text-white font-black text-xs`}>
                      ...
                      <div>
                        ...
                        {isPendingApproval && (
                          <div className="mt-1 inline-flex items-center gap-1.5 px-2 py-0.5 bg-red-100 border border-red-300 rounded-md">
                            <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse inline-block"></span>
                            <span className="text-[10px] font-black uppercase text-red-700">Esperando aprobar</span>
                          </div>
                        )}
                      </div>
                    </div>
