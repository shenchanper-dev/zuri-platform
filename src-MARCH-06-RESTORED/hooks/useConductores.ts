...
class ConductorRepository {
  ...
  async actualizarConductor(id: number, datos: Partial<Conductor>): Promise<Conductor> {
    ...
    // Ahora las fotos siempre vienen como strings (Base64 o URL), nunca como File
    // → Siempre usamos JSON, eliminando complejidad de FormData
  ...
  async asignarPin(id: number): Promise<{ pin: string; success: boolean }> {
    console.log(`🔐 [API] Asignando PIN temporal a conductor ${id}`);
    const response = await fetchWithTimeout(`${this.baseUrl}/${id}/asignar-pin`, {
      method: 'POST',
    });
...
export interface UseConductoresReturn {
  ...
  asignarPin: (id: number) => Promise<UseCaseResult<{ pin: string }>>;
...
export function useConductores(): UseConductoresReturn {
  ...
  const asignarPin = useCallback(async (id: number): Promise<UseCaseResult<{ pin: string }>> => {
    setLoading(true);
    setError(null);

    try {
      const result = await repository.asignarPin(id);

      if (result.success) {
        // Actualizar el estado local para reflejar que ahora tiene PIN
        setConductores(prev => prev.map(c => c.id === id ? { ...c, tienePin: true } : c));
        return { success: true, data: { pin: result.pin } };
      } else {
        throw new Error('Error al asignar PIN');
      }
    } catch (error: unknown) {
      let errorMessage = 'Error al asignar PIN';
