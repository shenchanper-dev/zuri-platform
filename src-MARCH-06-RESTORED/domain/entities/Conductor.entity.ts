...
export interface Conductor {
  ...
  // Real-Time GPS Tracking (Live Fleet Management)
  ubicacionActualLatitud?: number;
  ubicacionActualLongitud?: number;
  ultimaActualizacionGPS?: string;
  precisionGPS?: number;           // Accuracy in meters
  velocidadActual?: number;        // Speed in km/h
  rumboActual?: number;            // Heading 0-359 degrees
  nivelBateria?: number;           // Device battery 0-100
  estaConectado?: boolean;         // Is driver online?
  ...
  // Medical Equipment (NEMT Specific)
  equipamiento?: string[]; // ["OXIGENO", "SILLA_RUEDAS", "CAMILLA", "BOTIQUIN", "RAMPA"]
  ...
  añoVehiculo?: number;
  placaVehiculo?: string;             // Deprecated aliase
  marcaAuto?: string;                 // Deprecated aliase
  modeloAuto?: string;                // Deprecated aliase
  añoAuto?: number;                   // Deprecated aliase
...
export type NivelAprobacionConductor = 0 | 1 | 2 | 3 | 4;

export interface AprobacionConductor {
  nivel: NivelAprobacionConductor;
  aprobadoParaAceptarServicios: boolean;
  requisitosCumplidos: {
    dni: boolean;
    nombres: boolean;
    apellidos: boolean;
...
export const evaluarAprobacionConductor = (conductor: Partial<Conductor>): AprobacionConductor => {
  const dni = typeof conductor.dni === 'string' ? conductor.dni : '';
  const nombres = typeof conductor.nombres === 'string' ? conductor.nombres : '';
  const apellidos = typeof conductor.apellidos === 'string' ? conductor.apellidos : '';
  const celular1 = typeof conductor.celular1 === 'string' ? conductor.celular1 : '';
  const placa = typeof conductor.placa === 'string' ? conductor.placa : '';
  const licencia = typeof conductor.numeroBrevete === 'string' ? conductor.numeroBrevete : '';
  const servicios = Array.isArray(conductor.servicios) ? conductor.servicios : [];
  const equipamiento = Array.isArray(conductor.equipamiento) ? conductor.equipamiento : [];

  const dniOk = /^\d{8}$/.test(dni);
  const nombresOk = nombres.trim().length > 0;
  const apellidosOk = apellidos.trim().length > 0;
  const celularOk = celular1.replace(/\D/g, '').length >= 9;
  const placaOk = placa.trim().length >= 5;
  const licenciaOk = licencia.trim().length > 0;
  const serviciosOk = servicios.length > 0;
  const nemtBasicoOk = equipamiento.includes('OXIGENO') || equipamiento.includes('RAMPA');
  ...
  const requisitosCumplidos = {
    ...
    nemtBasico: nemtBasicoOk,
  };

  const faltantes = [
    !dniOk ? 'dni' : null,
    !nombresOk ? 'nombres' : null,
    !apellidosOk ? 'apellidos' : null,
    !celularOk ? 'celular' : null,
    !placaOk ? 'placa' : null,
    !licenciaOk ? 'licencia' : null,
    !serviciosOk ? 'servicios' : null,
    !nemtBasicoOk ? 'nemtBasico' : null,
  ].filter(Boolean) as string[];

  let nivel: NivelAprobacionConductor = 0;
  if (dniOk && nombresOk && apellidosOk) nivel = 1;
  if (nivel >= 1 && celularOk) nivel = 2;
  if (nivel >= 2 && placaOk && licenciaOk) nivel = 3;
  if (nivel >= 3 && serviciosOk && nemtBasicoOk) nivel = 4;

  return {
    nivel,
    aprobadoParaAceptarServicios: nivel >= 4 && conductor.estado === EstadoConductor.ACTIVO,
    requisitosCumplidos,
    faltantes,
  };
};
