...
export default function ConductorModalNEMT({
  ...
  // 🚀 SOLUCIÓN PUNTO 2: ENVÍO CON FORM DATA PARA PERSISTENCIA DE FOTOS
  const handleSubmit = async () => {
    if (!validate()) return;
    setIsSubmitting(true);

    try {
      const url = conductor ? `/api/conductores/${conductor.id}` : '/api/conductores';
      ...
      console.log('🎨 [handleSubmit] colorVehiculo value:', formData.colorVehiculo);

      Object.keys(formData).forEach(key => {
        const value = formData[key];

        // No enviar nulos o indefinidos
        if (value === null || value === undefined) return;

        // Si es un archivo (nueva foto), se agrega tal cual
        if (value instanceof File) {
          console.log(`📸 [handleSubmit] Adjuntando archivo nuevo: ${key}`);
          ...
          // Para foto y fotoVehiculo: solo omitir si es string vacío
          // Si tiene contenido (URL existente), enviarlo para que persista
          if (value === '' && (key === 'foto' || key === 'fotoVehiculo')) {
            console.log(`⚠️ [handleSubmit] Omitiendo ${key} vacío`);
            return;
          }

          // Si es foto/fotoVehiculo con URL (string no vacío), enviarlo
          if ((key === 'foto' || key === 'fotoVehiculo') && typeof value === 'string' && value.length > 0) {
            console.log(`✅ [handleSubmit] Enviando ${key} existente:`, value);
            data.append(key, value);
            return;
          }
